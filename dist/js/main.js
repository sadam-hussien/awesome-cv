...
$(function () {
  ...
  $("header .menu li").on("click", function () {
    $(this).addClass("active").siblings("li").removeClass("active"); // go to section
  ...
  btn.on("mousemove", function (e) {
    var coor = e.target.getBoundingClientRect();
    var x = e.clientX - coor.left;
    var y = e.clientY - coor.top;
    btn.css("--x", x + "px");
    btn.css("--y", y + "px");
  }); // init skill progress
  ...
  initprogress(".other_skills .chart"); // works mixitup

  var mixer = mixitup('.items_wrap'); // start animating
  // start header
  ...
  title_anim($(".contact"), 300, contactTitle); // start findElements function
  // start about section
  ...
  findElements($(".about"), 100, aboutbtn); // emd about
  // start services

  findElements($(".services"), 100, servicesitem); // end services
  // start skills

  findElements($(".skills"), 100, skillsmaindefination);
  findElements($(".skills"), -200, skillsframeworksdefination);
  findElements($(".skills"), -600, skillsotherdefination); // end skills
  // start works

  findElements($(".works"), 100, workstabs);
  findElements($(".works"), 100, worksitem); // end works
